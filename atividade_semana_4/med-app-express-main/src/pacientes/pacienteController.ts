...
export const consultaPorPacienteValidation = [
  query('userInput') 
  //query('userInput', 'O campo de busca é inválido.') // Mensagem de erro padrão
    .not().isNumeric().withMessage('E01: O campo de busca é inválido.') // Deve ser uma string
    .not().contains('=').withMessage('E02: O campo de busca é inválido.') // Bloquear =
    .not().contains('%').withMessage('E03: O campo de busca é inválido.') // Bloquear %
    .not().contains('/*').withMessage('E04: O campo de busca é inválido.') // Bloquear /*
    .not().contains('--').withMessage('E05: O campo de busca é inválido.') // Bloquear --
    .trim()     // Sanitização: remove espaços em branco do início e do fim
    .isLength({ min: 2, max: 80 }).withMessage('E10: O campo de busca é inválido.') // Deve ter entre 2 e 80 caracteres
    .escape()  // Sanitização: converte caracteres HTML (<, >, &, etc)
];
